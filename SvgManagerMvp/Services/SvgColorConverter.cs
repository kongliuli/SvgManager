...
namespace SvgManagerMvp.Services
{
    public class SvgColorConverter
    {
        ...
        public string NormalizeSvgColors(string svgContent, TargetColorFormat targetFormat = TargetColorFormat.Rgba)
        {
            if (string.IsNullOrWhiteSpace(svgContent))
                return svgContent;

            // 查找所有颜色值并替换
            foreach (var format in ColorPatterns.Keys)
            {
                svgContent = ColorPatterns[format].Replace(svgContent, match =>
                {
                    string originalColor = match.Value;
